...
class QuantValidator:
    ...
    def run_monte_carlo(self, metrics: dict, num_simulations: int = 100, drawdown_limit: float = 2.0) -> dict:
        ...
        # We model trade outcomes as binary (Win/Loss) with 50% Win Rate.
        # Win Return = profit_factor * 0.01 (1%)
        # Loss Return = -0.01 (-1%)
        win_rate = 0.50
        win_return = profit_factor * 0.01
        loss_return = -0.01
...
<body class="text-slate-100 font-sans p-6 md:p-12">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="glass-card rounded-2xl p-6 flex flex-col justify-between space-y-6">
                ...
                <div class="bg-slate-900/50 border border-slate-800 rounded-xl p-4 text-xs text-slate-400 space-y-2">
                    ...
                    <p>Paths are generated by shuffling historical trade returns modeled under a 50% Win Rate and aligned with the backtested Profit Factor of {pf}.</p>
                </div>
            </div>
    ...
    <script>
        ...
        const winReturn = profitFactor * 0.01;
